...
export function MenuItemCard({ style, item, theme, shadow, onAdd }: MenuItemCardProps) {
  const baseCard = clsx(
        ...
        <div className="relative flex h-full flex-col justify-end gap-2 p-6">
          ...
          <div className="flex items-center justify-between text-sm font-semibold">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                className="rounded-full bg-white/20 px-3 py-1 text-xs uppercase tracking-wide text-white ring-1 ring-inset ring-white/40 hover:bg-white/30"
        ...
        <div className="flex flex-1 flex-col gap-3 p-4" style={{ backgroundColor: theme.cardBackground, color: theme.text }}>
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <button
                className="rounded-full border border-orange-200 px-3 py-1 text-xs font-semibold text-orange-700 transition hover:border-orange-300"
                onClick={() => onAdd?.(item.id)}
              >
                + Add
              </button>
      ...
      <article className={clsx(baseCard, 'items-center justify-between p-4')} style={{ backgroundColor: theme.cardBackground }}>
        ...
        <div className="flex items-center gap-4">
          <div className="flex flex-col items-end gap-2">
            ...
            <button
              className="rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-700 hover:border-orange-300"
              onClick={() => onAdd?.(item.id)}
            >
              + Add
            </button>
  ...
  return (
    <article className={clsx(baseCard, 'p-3 sm:p-4')} style={{ backgroundColor: theme.cardBackground }}>
      {item.image && <img src={item.image} alt="" className="h-24 w-28 rounded-xl object-cover" />}
